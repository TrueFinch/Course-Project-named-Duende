...
class Model(object):
    ...
    def process_image(image):
        """
        Processing image for prediction. Saving in temporal folder so that it could be opened by PIL.
    ...
    def augment(image, label):
        """
        Augmenting image for training. Saving in temporal folder so that it could be opened by PIL.
        Cropping and scaling so that the longest side it 20.
        The width and height of the scaled image are used to resize it again so that there would be 4 images
        with different combinations of weight and height. Then putting it in a center of 28x28 blank image.
        12 possible angles are defined and each image is randomly rotated by 6 of these angles.
        Images converted to arrays and normalized.
